...
// copy constructor
template <typename T> Matrix<T>::Matrix(const Matrix<T> &that) {
  nRows = that.rows();
  nCols = that.cols();
...
template <typename T> Matrix<T> &Matrix<T>::operator=(const Matrix<T> &that) {
  if (this != &that) {
    nRows = that.nRows;
    nCols = that.nCols;
    numElements_ = that.numElements_;
...
// destructor
template <typename T> Matrix<T>::~Matrix() { delete[] mat; }

/* ------------- Very basic operations -------------- */
template <typename T> long Matrix<T>::rows() const { return nRows; }

template <typename T> long Matrix<T>::cols() const { return nCols; }

template <typename T> long Matrix<T>::size() const { return numElements_; }
...
template <typename T>
std::vector<std::tuple<long, long>>
Matrix<T>::getAllLocalWavevectors(BaseBandStructure &bandStructure) {
...
template <typename T> double Matrix<T>::squaredNorm() {
  double x = norm();
  return x * x;
}

template <typename T> T Matrix<T>::dot(const Matrix<T> &that) {
  T scalar = (T)0.;
  for (int i = 0; i < numElements_; i++) {
