...
// alias template
template <typename T> using ParallelMatrix = Matrix<T>;
...
ParallelMatrix<T>::ParallelMatrix(const int &numRows, const int &numCols,
  ...
  if (numRows_ % numBlocksRows_ != 0)
    blockSizeRows_ += 1;
  blockSizeCols_ = numCols_ / numBlocksCols_;
  if (numCols_ % numBlocksCols_ != 0)
    blockSizeCols_ += 1;
...
template <typename T> ParallelMatrix<T>::ParallelMatrix() {
  numRows_ = 0;
  numCols_ = 0;
  numLocalRows_ = 0;
  numLocalCols_ = 0;
...
template <typename T> ParallelMatrix<T>::~ParallelMatrix() { delete[] mat; }

template <typename T> long ParallelMatrix<T>::rows() const { return numRows_; }

template <typename T> long ParallelMatrix<T>::cols() const { return numCols_; }

template <typename T> long ParallelMatrix<T>::size() const {
  return cols() * rows();
}
...
template <typename T> void ParallelMatrix<T>::eye() {
...
template <typename T> T ParallelMatrix<T>::squaredNorm() { return dot(*this); }

template <typename T> T ParallelMatrix<T>::norm() {
  return sqrt(squaredNorm());
}

template <typename T> T ParallelMatrix<T>::dot(const ParallelMatrix<T> &that) {
...
template <typename T> ParallelMatrix<T> ParallelMatrix<T>::operator-() const {
