...
double zscal_(const int& n, const std::complex<double>&,
              std::complex<double>* x, const int&);
...
void dgemv_(const char& trans, const int& nr, const int& nc,
            const double& alpha, const double* amat, const int& lda,
            const double* bv, const int& incx, const double& beta, double* cv,
            const int& incy);
...
double zlange_(char* NORM, int* M, int* N, std::complex<double>* A, int* LDA,
               double* work);
