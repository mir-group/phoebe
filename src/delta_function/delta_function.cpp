...
void formTets(const Eigen::Vector3i & grid, TetraData & tetra) {
	...
	long counter,aux,ip1,jp1,kp1,ii,jj,kk;
	...
	tetra.qToTet = Eigen::Tensor<long,3>(n1*n2*n3,24,2);
...
void fillTetsEigs(const long & numBands, const Eigen::MatrixXd & energy,
	...
	for ( long it = 0; it < tetra.numTetra; it++ ) { //over tetrahedra
		for ( long ib = 0; ib < numBands; ib++ ) { //over bands
			...
			for ( long iv = 0; iv < 4; iv++) {
				tetra.tetraEigVals(it,ib,iv) = temp[iv];
			}
		}
	}
}
...
double fillTetsWeights(const double & energy, const long & ib, const long & iq,
	...
	for ( long i = 0; i < num; i++ ) {//over all tetrahedra
