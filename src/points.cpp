...
IrreduciblePoints::IrreduciblePoints(const IrreduciblePoints &that)
    : Points(that), mapReducibleToIrreducible(that.mapReducibleToIrreducible),
...
Point ActivePoints::getPoint(const long &index) { return Point(*this, index); }

Point PathPoints::getPoint(const long &index) { return Point(*this, index); }

Points ActivePoints::getParentPoints() { return parentPoints; }
...
long IrreduciblePoints::getNumPoints() { return numIrredPoints; }
  ...
  for (long ik = 0; ik < numPoints; ik++) {
    ...
    if (equiv(ik) == ik) {
      ...
      for (auto symm : symms) {
        ...
        inTheList = (abs(xx - round(xx)) <= eps && abs(yy - round(yy)) <= eps &&
                     abs(zz - round(zz)) <= eps);
        if (inTheList) {
          ix = mod(
              long(round(rotatedPoint(0) * mesh(0) - offset(0) + 2 * mesh(0))),
              mesh(0));
          iy = mod(
              long(round(rotatedPoint(1) * mesh(1) - offset(1) + 2 * mesh(1))),
              mesh(1));
          iz = mod(
              long(round(rotatedPoint(2) * mesh(2) - offset(2) + 2 * mesh(2))),
              mesh(2));
...
Eigen::VectorXi
IrreduciblePoints::getIndexReducibleFromIrreducible(const long &indexIrr) {
...
Eigen::Vector3d Point::getCoords(const int &basis, const bool &inWignerSeitz) {
  ...
  if (not inWignerSeitz) {
    Eigen::Vector3d crysCoords = points.getPointCoords(index, crystalCoords_);
...
double Point::getWeight() { return points.getWeight(index); }
