...
PhononH0::PhononH0(Crystal &crystal, const Eigen::MatrixXd &dielectricMatrix_,
        ...
        particle(Particle::phonon), bornCharges(bornCharges_), forceConstants(
                forceConstants_) {
            ...
            for (long n1 = -nr1Big; n1 <= nr1Big; n1++) {
                ...
                for (long n2 = -nr2Big; n2 <= nr2Big; n2++) {
                    ...
                    for (long n3 = -nr3Big; n3 <= nr3Big; n3++) {
                        ...
                        x = wsweight(r_ws, rws);
                        wscache_(n3ForCache, n2ForCache, n1ForCache, nb, na) =
                                x;
                        total_weight += x;
                    }
                }
...
void PhononH0::longRangeTerm(Eigen::Tensor<std::complex<double>, 4> &dyn,
    ...
    if (qCoarseGrid(0) == 1) {
        ...
        nr1x = (long) (sqrt(geg)
                / sqrt(
                        reciprocalUnitCell.col(0).transpose()
    ...
    if (qCoarseGrid(1) == 1) {
        ...
        nr2x = (long) (sqrt(geg)
                / sqrt(
                        reciprocalUnitCell.col(1).transpose()
    ...
    if (qCoarseGrid(2) == 1) {
        ...
        nr3x = (long) (sqrt(geg)
                / sqrt(
                        reciprocalUnitCell.col(2).transpose()
    ...
    for (long m1 = -nr1x; m1 <= nr1x; m1++) {
        for (long m2 = -nr2x; m2 <= nr2x; m2++) {
            for (long m3 = -nr3x; m3 <= nr3x; m3++) {
                g(0) = m1 * reciprocalUnitCell(0, 0)
                        + m2 * reciprocalUnitCell(0, 1)
                        + m3 * reciprocalUnitCell(0, 2);
                g(1) = m1 * reciprocalUnitCell(1, 0)
                        + m2 * reciprocalUnitCell(1, 1)
                        + m3 * reciprocalUnitCell(1, 2);
                g(2) = m1 * reciprocalUnitCell(2, 0)
                        + m2 * reciprocalUnitCell(2, 1)
                ...
                if (geg > 0. && geg / alph / 4. < gmax) {
                    ...
                    for (long na = 0; na < numAtoms; na++) {
                        ...
                        for (long i = 0; i < 3; i++) {
                            ...
                            fnat(i) = 0.;
                            for (long nb = 0; nb < numAtoms; nb++) {
                                arg = ((atomicPositions.row(na)
                                        - atomicPositions.row(nb)) * g).value();
            ...
            for (long n1 = -nr1Big; n1 < nr1Big; n1++) {
                ...
                for (long n2 = -nr2Big; n2 < nr2Big; n2++) {
                    ...
                    for (long n3 = -nr3Big; n3 < nr3Big; n3++) {
                        ...
                        weight = wscache(n3ForCache, n2ForCache, n1ForCache, nb,
                                na);
                        ...
                        if (weight > 0.) {
                            ...
                            for (long ipol = 0; ipol < 3; ipol++) {
                                for (long jpol = 0; jpol < 3; jpol++) {
                                    dyn(ipol, jpol, na, nb) += (forceConstants(
                                            m1, m2, m3, ipol, jpol, na, nb)
...
std::tuple<Eigen::VectorXd, Eigen::MatrixXcd> PhononH0::dyndiag(
    ...
    for (long iat = 0; iat < numAtoms; iat++) {
        for (long jat = 0; jat < numAtoms; jat++) {
            for (long ipol = 0; ipol < 3; ipol++) {
                for (long jpol = 0; jpol < 3; jpol++) {
                    dyn2Tmp(iat * 3 + ipol, jat * 3 + jpol) = dyn(ipol, jpol,
                            iat, jat);
                }
            }
        }
    ...
    for (long iat = 0; iat < numAtoms; iat++) {
        ...
        for (long jat = 0; jat < numAtoms; jat++) {
            ...
            for (long ipol = 0; ipol < 3; ipol++) {
                for (long jpol = 0; jpol < 3; jpol++) {
                    dyn2(iat * 3 + ipol, jat * 3 + jpol) /= sqrt(
                            speciesMasses(iType) * speciesMasses(jType));
                }
            }
        }
    }
...
void PhononH0::setAcousticSumRule(const std::string &sumRule) {
    ...
    if (sr == "simple") {
                        ...
                        for (long n1 = 0; n1 < qCoarseGrid(0); n1++) {
                            for (long n2 = 0; n2 < qCoarseGrid(1); n2++) {
                                for (long n3 = 0; n3 < qCoarseGrid(2); n3++) {
                                    sum += forceConstants(n1, n2, n3, i, j, na,
                                            nb);
                                }
                            }
                        }
        ...
        Eigen::Tensor<double, 8> uvec(18 * numAtoms, nr1, nr2, nr3, 3, 3,
                numAtoms, numAtoms);
        uvec.setZero();

        Eigen::Tensor<double, 7> frc_new(nr1, nr2, nr3, 3, 3, numAtoms,
                numAtoms);
        ...
        Eigen::Tensor<long, 3> ind_v(9 * numAtoms * numAtoms * nr1 * nr2 * nr3,
                2, 7);
        Eigen::Tensor<double, 2> v(9 * numAtoms * numAtoms * nr1 * nr2 * nr3,
                2);
        v.setZero();
        ind_v.setZero();
                            ...
                            for (long na = 0; na < numAtoms; na++) {
                                for (long nb = 0; nb < numAtoms; nb++) {
                                    ...
                                    if (q != 0) {
                                        ...
                                        ind_v(m, 0, 6) = nb;
                                        v(m, 0) = 1. / sqrt(2.);
                                        ind_v(m, 1, 0) = mod((nr1 + 1 - n1),
                                                nr1);
                                        ind_v(m, 1, 1) = mod((nr2 + 1 - n2),
                                                nr2);
                                        ind_v(m, 1, 2) = mod((nr3 + 1 - n3),
                                                nr3);
                                        ind_v(m, 1, 3) = j;
                                        ind_v(m, 1, 4) = i;
                                        ind_v(m, 1, 5) = nb;
                    ...
                    for (long n3 = 0; n3 < nr3; n3++) {
                        for (long i = 0; i < 3; i++) {
                            for (long j = 0; j < 3; j++) {
                                for (long na = 0; na < numAtoms; na++) {
                                    for (long nb = 0; nb < numAtoms; nb++) {
                                        w(n1, n2, n3, i, j, na, nb) = uvec(k,
                                                n1, n2, n3, i, j, na, nb);
                                        x(n1, n2, n3, i, j, na, nb) = uvec(k,
                                                n1, n2, n3, i, j, na, nb);
                                    }
                                }
                            }
                    ...
                    for (long n1 = 0; n1 < nr1; n1++) {
                        for (long n2 = 0; n2 < nr2; n2++) {
                            for (long n3 = 0; n3 < nr3; n3++) {
                                for (long j = 0; j < 3; j++) {
                                    for (long nb = 0; nb < numAtoms; nb++) {
                                        scal += x(n1, n2, n3, i1, j, na1 - 1,
                                                nb)
                                                * uvec(q, n1, n2, n3, i1, j,
                                                        na1 - 1, nb);
                                    }
                                }
                            }
                        ...
                        for (long n3 = 0; n3 < nr3; n3++) {
                            for (long i = 0; i < 3; i++) {
                                for (long j = 0; j < 3; j++) {
                                    for (long na = 0; na < numAtoms; na++) {
                                        for (long nb = 0; nb < numAtoms; nb++) {
                                            scal += uvec(k, n1, n2, n3, i, j,
                                                    na, nb)
                                                    * frc_new(n1, n2, n3, i, j,
                                                            na, nb);
                                        }
                                    }
                                }
