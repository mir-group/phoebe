...
#ifdef MPI_AVAIL
// This function to create a reduction op for user defined types
// is essentially borrowed from the heroic BigMPI project authored by Jeff Hammond,
// but altered to work just for summing complex doubles, as that's all we need
// for Phoebe at this time.
// It could be made more general, or another function could be made for another
// type if necessary.
// see https://github.com/jeffhammond/BigMPI/blob/master/src/reductions_x.c
void BigMPI_SUM_CDOUBLE_x(void * invec, void * inoutvec, int * len, MPI_Datatype * bigtype)
{
    // reducing a single element of bigtype
    assert(*len==1);
