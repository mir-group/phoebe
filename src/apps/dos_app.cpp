...
void PhononDosApp::run(Context &context) {
    ...
    FullBandStructure fullBandStructure = phononH0.populate(fullPoints,
...
void ElectronWannierDosApp::run(Context &context) {
    ...
    bool withVelocities = false;
    bool withEigenvectors = false;
    FullBandStructure fullBandStructure = h0.populate(fullPoints,
            withVelocities, withEigenvectors);
    ...
    for (long i = 0; i < numEnergies; i++) {
        outfile << energies[i] * energyRyToEv << "\t" << dos[i] / energyRyToEv
                << "\n";
    }
