...
std::tuple<std::vector<double>, std::vector<long>>
Window::apply(Eigen::VectorXd &energies) {
  ...
  if (method == population) {
    ...
    for (long ib = 0; ib < numBands; ib++) {
      dndtMin(ib) =
          particle.getDndt(energies(ib), temperatureMax, chemicalPotentialMin);
      dndtMax(ib) =
          particle.getDndt(energies(ib), temperatureMax, chemicalPotentialMin);
      dndeMin(ib) =
          particle.getDnde(energies(ib), temperatureMax, chemicalPotentialMin);
      dndeMax(ib) =
          particle.getDnde(energies(ib), temperatureMax, chemicalPotentialMin);
    }
...
std::tuple<std::vector<double>, std::vector<long>>
Window::internalPopWindow(Eigen::VectorXd &energies, Eigen::VectorXd &dndtMin,
...
std::tuple<std::vector<double>, std::vector<long>>
Window::internalEnWindow(Eigen::VectorXd energies) {
...
long Window::getMethodUsed() { return method; }
